...
5. `POST /game/update` (статус аккаунта) отправляется:
   ...
   Повторяющиеся идентичные статусы между этими событиями не отправляются. При сравнении статусов поля `time_1..time_5` исключаются (они тикают каждую секунду и иначе форсируют отправку каждый тик).
