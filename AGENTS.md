...
5. `POST /game/update` (статус аккаунта) отправляется:
   ...
   Повторяющиеся идентичные статусы между этими событиями не отправляются. При сравнении статусов поля `time_1..time_5` (тикают каждую секунду) и `custom_data.position_y` (шумит от прыжков/неровностей) исключены — изменение только их не триггерит отправку.
