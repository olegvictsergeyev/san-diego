...
- Для пережития телепорта используем глобальный `queue_on_teleport`: загрузчик `final/agent.lua` при каждом запуске первым делом (до guard'а и любых рискованных операций) ставит в очередь перезапуск самого себя. Очередной код ждёт `game:IsLoaded()`, делает до 5 попыток с бэкофом и верифицирует старт по `getgenv().SanDiegoAgentLastStartJobId == game.JobId`. Это покрывает любой телепорт (не только `join_private_server`). Если `queue_on_teleport` недоступен — полагаться на `autoexec`.
