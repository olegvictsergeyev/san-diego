...
- Для пережития телепорта используем глобальный `queue_on_teleport`: загрузчик `final/agent.lua` при запуске ставит в очередь перезапуск самого себя **только если JobId сменился** (`SanDiegoAgentLastStartJobId ~= game.JobId`). Очередной код ждёт `game:IsLoaded()`, делает до 12 попыток с бэкофом (2/4/8… с, потолок 300 с) и верифицирует старт по `getgenv().SanDiegoAgentLastStartJobId == game.JobId`. Если `queue_on_teleport` недоступен — полагаться на `autoexec`. Важно: постановка в очередь без проверки JobId приводит к бурной размножению копий — каждый запуск лоадера добавлял бы ещё одну, и при первом телепорте исполнились бы все разом.
