...
7. **Протестировать**:
   ...
   - При ошибке 277/278 (дисконнект) должен отправляться лог в `custom_data.disconnect` и, если включено, нажиматься кнопка Reconnect.
...
5. `POST /game/update` (статус аккаунта) отправляется только при изменении данных из лога: позиции, баланса, статуса, текущей команды и т.д. Повторяющиеся идентичные статусы не отправляются.
