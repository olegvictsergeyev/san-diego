...
1. Полноценный Orion UI удалён по решению владельца: вместо него агент показывает только компактный бейдж с версией (`SanDiegoAgentVersion`) в левом нижнем углу. Номер версии бейджа берётся из `CONFIG.version` и обновляется автоматически при бампе версии.
...
5. `POST /game/update` (статус аккаунта) отправляется:
   ...
   - при старте агента;
   - принудительно раз в минуту, если не было других отправок (так бэкенд получает свежие `time_1..time_5`).
   Повторяющиеся идентичные статусы между этими событиями не отправляются. При сравнении статусов поля `time_1..time_5` (тикают каждую секунду) и `custom_data.position_y` (шумит от прыжков/неровностей) исключены — изменение только их не триггерит отправку. Пока агент выполняет команду, из сравнения дополнительно исключены `custom_data.position_x`/`position_z` — позиция во время движения не порождает `data_changed`; одна отправка с финальной позицией выполняется по завершении команды (существующий force-send).
   В корень payload каждого `/game/update` добавляется `update_reason` (`initial` | `data_changed` | `forced` | `heartbeat`), а для `data_changed` — ещё `update_fields` со списком изменившихся полей (например `custom_data.balance`). Бэкенд дополнительные поля корня игнорирует (проверено тестом, 200 OK).
...
- Для пережития телепорта используем глобальный `queue_on_teleport`: загрузчик `final/agent.lua` при запуске ставит в очередь перезапуск самого себя **только если JobId сменился** (`SanDiegoAgentLastStartJobId ~= game.JobId`). Очередной код ждёт `game:IsLoaded()`, делает до 12 попыток с бэкофом (2/4/8… с, потолок 300 с) и верифицирует старт по `getgenv().SanDiegoAgentLastStartJobId == game.JobId`. Первая исполнившаяся копия «захватывает» JobId через `getgenv().SanDiegoAgentTeleportHandledJobId` (атомарно, без приостановок между проверкой и записью) — при бурсте из очереди работает ровно одна копия. Если `queue_on_teleport` недоступен — полагаться на `autoexec`. Важно: постановка в очередь без проверки JobId приводит к бурной размножению копий — каждый запуск лоадера добавлял бы ещё одну, и при первом телепорте исполнились бы все разом.
