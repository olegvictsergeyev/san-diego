--[[
    San Diego Agent — Test v2: run place_all_printers command standalone
    ================================================================
...
local function copy()
    ...
    if writefile then pcall(function() writefile("place_all_printers_test_v2_log.txt", text) end) end
end

log("========== PLACE ALL PRINTERS TEST V2 ==========")
...
if result.data then
    ...
    for k, v in pairs(result.data) do
        if k == "errors" and typeof(v) == "table" then
            log("  errors:")
