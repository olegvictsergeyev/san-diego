--[[
    San Diego Agent — Standalone test v3.1: debug version
    =====================================================
    То же, что v3, но с обёрткой pcall на каждом шаге и максимальным
    логированием, чтобы найти место зависания/падения.
]]
...
local function logError(label, err)
    local msg = "[ERROR " .. tostring(label) .. "]: " .. tostring(err)
    log(msg)
    warn(msg)
end
...
local function copy()
    ...
    if writefile then pcall(function() writefile("printer_grid_test_v3_1_log.txt", text) end) end
end
...
local function getCharacter()
    log("[getCharacter] waiting for character...")
    local char = player.Character or player.CharacterAdded:Wait()
    log("[getCharacter] got character:", char and char.Name or "nil")
    return char
end
...
local function getHumanoid()
    ...
    log("[getHumanoid] found:", hum and "yes" or "no")
    return hum
end
...
local function getHrp()
    ...
    log("[getHrp] found:", hrp and "yes" or "no")
    return hrp
end
...
local function getPlayerPos()
    ...
    log("[getPlayerPos] pos=", pos and tostring(pos) or "nil")
    return pos
end
...
local function resetInventory()
    ...
    log("[resetInventory] backpack found")
    ...
    task.wait(0.3)
    log("[resetInventory] scan workspace for stray tools")
...
local function getToolSpacing(tool)
    ...
    if part and part:IsA("BasePart") then
        ...
        log("[getToolSpacing] part size=", tostring(size), "spacing=", maxSize + 0.5)
        return maxSize + 0.5
    end
...
local function findMoneyPrintersFolders()
    ...
    scan(Workspace, 0)
    log("[findMoneyPrintersFolders] total found:", tostring(#folders))
    return folders
end

local function findFloorInUnit(unit)
    if not unit then
        log("[findFloorInUnit] unit is nil")
        return nil
    end
    log("[findFloorInUnit] unit=", unit.Name)
    local best = nil
    local bestArea = 0
    local all = unit:GetDescendants()
    log("[findFloorInUnit] descendants=", tostring(#all))
    ...
    if best then
        log("[findFloorInUnit] best floor=", best.Name, "size=", tostring(best.Size), "area=", tostring(bestArea))
    else
        log("[findFloorInUnit] no floor found")
    end
    return best
end
...
local function isPlayerOnFloor(playerPos, floor)
    ...
    local onFloor = math.abs(localPos.Y) <= halfY + 8
        and localPos.X >= -halfX - 1 and localPos.X <= halfX + 1
        and localPos.Z >= -halfZ - 1 and localPos.Z <= halfZ + 1
    log("[isPlayerOnFloor] local=", tostring(localPos), "onFloor=", tostring(onFloor))
    return onFloor
end
...
local function chooseTargetFolder()
    ...
    if not playerPos then
        log("[chooseTargetFolder] playerPos is nil")
        return nil, nil
    end
    ...
    for i, folder in ipairs(folders) do
        log("[chooseTargetFolder] checking folder #", tostring(i), folder:GetFullName())
        local unit = folder.Parent
        local floor = nil
        local ok, err = pcall(function()
            floor = findFloorInUnit(unit)
        end)
        if not ok then
            logError("findFloorInUnit", err)
            continue
        end
        if floor and isPlayerOnFloor(playerPos, floor) then
            log("[chooseTargetFolder] MATCH:", folder:GetFullName())
            local count = 0
            local ok2, err2 = pcall(function()
                count = #getExistingPrinterPositions(folder)
            end)
            if not ok2 then
                logError("countExisting", err2)
                count = 0
            end
            return folder, count
        end
    ...
    log("[chooseTargetFolder] no floor match, fallback to nearest")
    local scored = {}
    for _, folder in ipairs(folders) do
        ...
        if not ok then
            logError("findFloorInUnit2", err)
            continue
        end
        local dist = math.huge
        local positions = {}
        local ok2, err2 = pcall(function()
            positions = getExistingPrinterPositions(folder)
    ...
    local best = scored[1]
    if not best then
        log("[chooseTargetFolder] no scored folders")
        return nil, nil
    end
    if best.dist > SAFETY_MAX_DIST_FROM_PLAYER then
        log("[chooseTargetFolder] best folder too far:", round(best.dist, 1))
        return nil, nil
    end
    local count = 0
    pcall(function() count = #getExistingPrinterPositions(best.folder) end)
    return best.folder, count
end
...
local function analyzeGrid(positions)
    log("[analyzeGrid] positions=", tostring(#positions))
    ...
    log("[analyzeGrid] origin=", tostring(origin), "rowSpacing=", rowSpacing, "colSpacing=", colSpacing, "perRow=", #rows[1].points, "rows=", #rows)
...
local function buildGridFromFloor(floor, spacing)
    log("[buildGridFromFloor] floor=", floor.Name, "size=", tostring(floor.Size))
    local size = floor.Size
    local cf = floor.CFrame
...
log("========== PRINTER GRID PLACEMENT TEST v3.1 ==========")
...
local function main()
    ...
    if not ok then logError("findFloor", err) end
    if floor then
