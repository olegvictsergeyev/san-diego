--[[
    San Diego Agent — Standalone test: place all printers in apartment grid
    =======================================================================
    Автономный скрипт для тестирования расстановки всех принтеров в квартире.
    - Находит папку MoneyPrinters ближайшую к персонажу.
    - Анализирует уже стоящие принтеры и строит сетку (ряды/колонны).
    - Расставляет оставшиеся принтеры из инвентаря в свободные ячейки.
    - Защита от последствий предыдущих запусков: сбрасывает экипировку,
      возвращает в Backpack Tool'ы, оказавшиеся в Workspace.
    - Работает итеративно: если предыдущий запуск упал на середине,
      следующий продолжит с того же места.
    Запусти в executor'е и пришли JSON из консоли.
]]
...
local function copy()
    ...
    if writefile then pcall(function() writefile("printer_grid_test_log.txt", text) end) end
end

local PRINTER_NAME = "Money Printer"
local MAX_PRINTERS = 50
local PLACE_DELAY = 0.6
local SAFETY_MAX_DIST_FROM_PLAYER = 60
...
local function resetInventory()
    -- Возвращаем все экипированные Tool'ы в Backpack.
    local char = getCharacter()
    ...
    task.wait(0.3)
    -- Если в Workspace лежит Tool с нашим именем не в MoneyPrinters — вернём в Backpack.
    local function scan(parent)
        for _, c in ipairs(parent:GetChildren()) do
            if c:IsA("Tool") and c.Name == PRINTER_NAME then
                -- Проверим, не находится ли он уже в MoneyPrinters
...
local function chooseTargetFolder()
    ...
    -- Приоритет: папка с уже стоящими принтерами, ближайшая к игроку.
    local scored = {}
    for _, folder in ipairs(folders) do
        local pos = getFolderPosition(folder)
        local dist = pos and (pos - playerPos).Magnitude or math.huge
        local existing = 0
        ...
        -- Скор: чем ближе и чем больше принтеров, тем лучше.
        local score = dist - existing * 10
        table.insert(scored, { folder = folder, dist = dist, existing = existing, score = score })
    end
    table.sort(scored, function(a, b) return a.score < b.score end)

    local best = scored[1]
    if best.dist > SAFETY_MAX_DIST_FROM_PLAYER then
        log("[ChooseFolder] Closest folder too far:", round(best.dist, 1))
        return nil, nil
    end
...
local function analyzeGrid(positions)
    if #positions == 0 then
        return nil, "no existing printers"
    end
    ...
    -- Определяем ориентацию: пробуем группировку по X и по Z.
    ...
    -- Считаем строки/колонны. Больше кластеров по одной оси = меньше строк по ней.
    -- Ряды идут вдоль оси с меньшим числом кластеров.
    ...
    -- Сортируем строки по координате строки.
    table.sort(rows, function(a, b) return a.center < b.center end)

    -- Внутри каждой строки сортируем по колонке.
    ...
    -- spacing вдоль колонки (внутри строки)
    local colDists = {}
    for _, row in ipairs(rows) do
    ...
    -- Определяем высоту пола как средний Y.
    local sumY = 0
    for _, p in ipairs(pts) do sumY += p.y end
    local floorY = sumY / #pts

    -- Определяем направление сетки: угол от первой строки в сторону последней.
    local firstRow = rows[1]
    local lastRow = rows[#rows]
    local dirRow = Vector3.new(0, 0, 0)
    if rowAxis == "x" then
        dirRow = Vector3.new(1, 0, 0)
    else
        dirRow = Vector3.new(0, 0, 1)
    end
    local dirCol = Vector3.new(0, 0, 0)
    if colAxis == "x" then
        dirCol = Vector3.new(1, 0, 0)
    else
        dirCol = Vector3.new(0, 0, 1)
    end

    -- Знаки направлений: от минимальной координаты к максимальной.
    -- (если сетка растёт в обратную сторону — корректируем через colDirSign/rowDirSign позже)
    local firstPoint = firstRow.points[1]
    ...
    return {
        ...
        dirCol = dirCol,
        floorY = floorY,
        printersPerRow = #firstRow.points,
        rowCount = #rows,
...
local function generateNextSlots(grid, maxCount)
    ...
    -- Отмечаем занятые ячейки по координатам, округлённым к шагу.
    ...
    while placed < maxCount do
        -- Следующая ячейка: сначала добиваем текущую строку, потом следующая.
        ...
        local k = key(pos)
        if occupied[k] then
            -- Защита от зацикливания: если ячейка занята, сдвигаемся вправо.
            colIndex += 1
            if colIndex >= grid.printersPerRow then
                rowIndex += 1
                colIndex = 0
            end
            placed += 1 -- считаем попытку, чтобы не зависнуть
        else
...
log("========== PRINTER GRID PLACEMENT TEST ==========")
...
for i, pos in ipairs(slots) do
    -- local cancelled = false -- standalone, no external cancel
    -- if cancelled then
    --     log("Cancelled at slot", tostring(i))
    --     break
    -- end
    ...
    if ok then
        ...
        -- Возвращаем обратно в Backpack, чтобы не потерять.
