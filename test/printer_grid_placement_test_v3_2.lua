--[[
    San Diego Agent — Standalone test v3.2: raycast-based room detection
    ====================================================================
    Исправления:
    - Комната определяется по raycast'у вниз от персонажа (а не по самой
      большой детали в юните).
    - Дедупликация найденных MoneyPrinters-папок.
    - Убран `continue`, переписано совместимо со всеми executor'ами.
...
local function copy()
    ...
    if writefile then pcall(function() writefile("printer_grid_test_v3_2_log.txt", text) end) end
end
...
local function chooseTargetFolder()
    ...
    -- Способ 1: raycast вниз от игрока, найти деталь пола, затем подняться к юниту и его MoneyPrinters.
    ...
    -- Способ 2: найти пол внутри каждого юнита, на котором стоит игрок.
    ...
    -- Способ 3: ближайшая папка к игроку.
...
log("========== PRINTER GRID PLACEMENT TEST v3.2 ==========")
