...
local function runDemo()
    ...
    local ok, Material = pcall(function()
        local src = game:HttpGet("https://raw.githubusercontent.com/Kinlei/MaterialLua/master/Module.lua")
        -- Обходим автоматическое определение executor'а, которое может выбрать неподдерживаемый protect-метод.
        src = src:gsub("ProtectFunctions%[GetExploit%(%)%]%([^)]+%);", "NewInstance.Parent = CoreGuiService;")
