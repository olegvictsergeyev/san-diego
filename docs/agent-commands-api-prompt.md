...
- Если команда завершилась ошибкой, но успела накопить данные (например `drive`), `result` — JSON-строка вида `{ "error": "<текст ошибки>", "data": { ... } }`: читайте `error` в первую очередь, `data` — телеметрия заезда. Для статуса `cancelled` формат прежний — чистый `data`-JSON без поля `error`.
  ...
  "commands": [
    ...
    },
    {
      ...
      "params": {
        ...
        "wait_seconds": { "type": "number", "min": 0, "max": 60 }
      }
    },
    {
