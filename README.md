...
- `place_printer` — установить один Money Printer в своей комнате (экипировка + активация кликом, сервер ставит модель на 4 ст перед персонажем). Без параметров — перед взглядом; с `x`/`z` — в точке. Требует стоять в своей комнате.
- `place_all_printers` — автоматически расставить Money Printer сеткой в текущей комнате: комната и её границы определяются автоматически, начало от левого угла стены, на которую смотрит персонаж, колонки вправо, ряды вглубь, без выхода за границы, допускается частичное наложение. Параметр `max_total` (1..50, по умолчанию 50).
