...
- `move_to` — переместить персонажа к целевым координатам `x` и `z` (высота Y подстраивается под рельеф). Опциональный параметр `speed` (1..10, по умолчанию 10). Та же защита от препятствий, что у `move_x`/`move_z`.
