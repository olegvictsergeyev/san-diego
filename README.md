...
- `drive` — наземная езда на полной скорости (мотоцикл/машина). Параметр `x` (необязательный, −20000..20000) — смещение по X со знаком; разгон без ограничения скорости (физически ~630 ст/с на мотоцикле), резкое торможение с остановкой у цели. Параметр `z` (необязательный) — абсолютная координата полосы, по умолчанию 150.07 (главный проспект); если `z` не передан — выравнивается на полосу и придерживается её. Если `x` не передан — только встать в полосу, не уезжая. Результат: `travelled`, `vmax`, `brake_dist`, `lane_z`, `time` (опционально `t_300`). Требует сидеть в транспорте.
- `jump` — подпрыгнуть.
