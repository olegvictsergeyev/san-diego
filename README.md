...
- `nav_car` — перемещение летающей машины на смещение: параметры `x` и `z` (обязательные, −2000..2000) — расстояние в стадах со знаком направления. Скорость фиксированная 25 ст/с, высота +10 над поверхностью (максимум безопасной). Если активна сессия `fly_car` — после прибытия вернётся в зависание на прежней высоте, иначе сядет. Требует сидеть в машине.
- `jump` — подпрыгнуть.
