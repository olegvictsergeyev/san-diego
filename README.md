...
- `transfer_money_via_respawn` — передавать деньги целевому игроке, умирая и воскрешаясь, пока его баланс не достигнет заданной суммы. Перед смертью подбегает к цели, чтобы деньги падали рядом. Параметры: `identifier`, `amount`, `max_attempts`, `wait_seconds`, `move_to_target` (`on`/`off`, по умолчанию `on`).
- `jump` — подпрыгнуть.
