...
- `place_all_printers` — взять все Money Printer из инвентаря и расставить сеткой в ближайшей квартире. Персонаж должен стоять в верхнем левом углу комнаты и быть направленным внутрь комнаты (вдоль оси, по которой будет первая строка). Параметры `max_total` (1..50, по умолчанию 50) и `max_distance` (10..500, по умолчанию 200).
- `jump` — подпрыгнуть.
