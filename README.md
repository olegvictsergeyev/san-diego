...
- `drive` — наземная езда на полной скорости (мотоцикл/машина). Управление рулением: техника разворачивается носом к курсу (AlignOrientation) и тянется только вдоль продольной оси — z корректируется поворотами руля, бокового скольжения нет. Если техника стоит боком — сначала развернётся к курсу, потом поедет. Параметр `x` (необязательный, −20000..20000) — смещение по X со знаком; разгон без ограничения скорости (физически ~630 ст/с на мотоцикле), резкое торможение с остановкой у цели. Параметр `z` (необязательный) — абсолютная координата полосы, по умолчанию 150.07 (главный проспект); если `z` не передан — выравнивается на полосу и придерживается её. Если `x` не передан — только встать в полосу, не уезжая. В конце — выпрямление по оси дороги и активная остановка (техника иначе сохраняет импульс и укатится сама). Результат: `travelled`, `vmax`, `brake_dist`, `lane_z`, `time` (опционально `t_300`). Требует сидеть в транспорте.
- `jump` — подпрыгнуть.
