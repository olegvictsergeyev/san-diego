...
- `place_printer` — взять Money Printer из инвентаря и установить на пол в ближайшей квартире. Опциональные параметры `x`, `y`, `z`.
- `place_all_printers` — взять все Money Printer из инвентаря и расставить сеткой в ближайшей квартире, продолжая существующий порядок. Параметры `max_total` (1..50, по умолчанию 50) и `max_distance` (10..500, по умолчанию 200).
- `jump` — подпрыгнуть.
