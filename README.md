...
- `nav_car` — перемещение летающей машины на смещение: параметры `x` и `z` (обязательные, −2000..2000) — расстояние в стадах со знаком направления. По оси Y машина не смещается: летит на текущей высоте (при активной сессии `fly_car` — на её высоте), повторяя рельеф. Скорость фиксированная 20 ст/с. Прерывается `cancel`: движение остановится, при активной сессии `fly_car` машина останется висеть, иначе сессия завершится. При застревании (препятствие или физика перестала реплицироваться) движение аварийно прекращается с ошибкой `stuck`. Требует сидеть в машине.
- `car_state` — диагностика положения машины: на поверхности или закопана. Возвращает `state` (`on_ground` | `buried` | `airborne` | `unknown`), `ride_height` (высота Root над поверхностью, естественная ~2.2), `wheel_min`/`wheel_max` (зазор колёс, естественный ~+1.3; `wheel_min` < 0.2 => колесо под землёй), `position`, `speed`. Требует сидеть в машине.
- `jump` — подпрыгнуть.
