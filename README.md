...
- `place_all_printers` — убрать все уже стоящие Money Printer в квартире и разложить их заново плотной сеткой внутри комнаты. Использует встроенные координаты комнаты (замер уже выполнен, измерять повторно не нужно). Вмещает до 50 принтеров; если Tool'ов меньше — выкладывает все. Параметры `max_total` (1..50, по умолчанию 50) и `max_distance` (10..500, по умолчанию 200).
