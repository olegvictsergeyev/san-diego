...
- `transfer_money_via_respawn` — передавать деньги целевому игроку, умирая и воскрешаясь, пока его баланс не достигнет заданной суммы. Параметры: `identifier` (ник/display_name/user_id), `amount` (целевой баланс), `max_attempts` (опционально, по умолчанию 100), `wait_seconds` (опционально, по умолчанию 5).
- `jump` — подпрыгнуть.
