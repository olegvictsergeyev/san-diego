...
-- 2026-09-11 игра ужесточила проверку: на массовых заездах сервер
-- обнуляет скорость уже на ~405 ст/с (vmax=406 → knock в логах).
-- 2026-09-12: у части агентов (перезапущенные клиенты, хуже сеть)
-- knock фиксируется уже на 133-144 ст/с. Кап снижен до 120 — это
-- «порог 130 − запас»; на капе 120 все заезды чистые (7015 стад,
-- jumped, без knock). Уточнение по замеру probe — см. PROBE_* ниже.
