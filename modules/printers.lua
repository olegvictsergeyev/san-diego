...
Printers.MAX_BUY = 50
-- Лимит принтеров в ОДНОЙ комнате по правилам игры — 50.
-- (Расхождение placed vs room_total наблюдалось 08.09, но оказалось
-- НЕ серверным лимитом: «лишние» тихо пропадали из-за фейлов постановки
-- на лагающих мобильных клиентах. Сетка всегда целится ровно в этот лимит,
-- а фактический итог честно меряем room_total после паузы на репликацию.)
Printers.MAX_ROOM_PRINTERS = 50
...
-- считать покупку неудавшейся (нет денег / лаг сервера). 5 с — запас под
-- медленную репликацию на мобильных клиентах (на ПК подтверждение ~0.15 с).
Printers.BUY_CONFIRM_TIMEOUT = 5
...
function Printers:_pickupOne(model, isCancelled)
	...
	for attempt = 1, 3 do
		...
		-- репликация позиции на сервер: на мобиле сервер видит новое место
		-- персонажа до ~1 с; если кликнуть раньше — серверный proximity-check
		-- откажет и попытка сгорит впустую (полный ретрай ~4 с).
		task.wait(1.1)
...
function Printers:placeRoomGrid(maxTotal, isCancelled)
	...
	-- Честный фактический итог: после последней постановки модель может
	-- доехать по репликации с лагом (особенно на мобиле) — пауза 3 с и
	-- пересчёт. trimmed_by_server=true означает, что часть моделей исчезла
	-- после постановки (если true систематически — копать placement, а не
	-- «лимит игры»).
	task.wait(3)
