...
function CommandEngine:getCommandsSpec()
	return {
		...
		{
			name = "open_door",
			description = "Открыть дверь номера: тот же серверный вызов, что кнопка Open Door (без нажатия E). Если дверь уже открыта — ничего не делает, возвращает успех с open=true. Выбор двери: приоритет своей парадной, иначе ближайшая Interior (до 20 ст). Ретраи до 6 попыток: ожидание ApartmentDoorBusy перед каждой попыткой, верификация состояния 5 с, перед закрытием персонаж отшагивает из проёма",
			params = {},
		},
		{
			name = "close_door",
			description = "Закрыть дверь номера: тот же серверный вызов, что кнопка Close Door (без нажатия E). Если дверь уже закрыта — ничего не делает, возвращает успех с open=false. Выбор двери: приоритет своей парадной, иначе ближайшая Interior (до 20 ст). Ретраи до 6 попыток: ожидание ApartmentDoorBusy перед каждой попыткой, верификация состояния 5 с, перед закрытием персонаж отшагивает из проёма",
			params = {},
		},
		{
		...
		},
		{
			...
			params = {
				...
				speed = {
					...
					min = 0,
					max = 10,
					description = "Ограничение скорости по шкале 0..10: линейно 0..360 ст/с (10 или отсутствует = полная; 0 = не уезжать, только встать в полосу)",
				},
				jump_off = {
					type = "boolean",
					required = false,
